# Hi, I'm Jonathan Rincon 👋
### Senior VR Developer | Unity (7+ years) & Unreal Engine (2+ years)

Experienced Lead Developer specializing in high-performance XR solutions, multiplayer systems, and AI-driven interactions. I have a proven track record of delivering published applications on the Meta Store and high-end training simulators for global industries.

---

## 🚀 Featured Projects

### 🎓 Enterprise Training & Simulation

#### **Nurse & Doctor Clinical Training (AI-Driven)**
<p align="left">
  <img src="https://github.com/ringorjo/VirtualRealityProjects/blob/main/gif/AI.gif" width="500px">
</p>

* **Role:** Lead Developer (Architecture & AI Integration)
* **Stack:** Unity, OpenAI Realtime, ElevenLabs, Low-latency LLM Systems.
* **Description:** Developed a cutting-edge interaction system using real-time AI to create role-specific virtual patients. This project realistically simulates clinical conditions, enhancing immersion and training effectiveness for healthcare professionals.
* [**Watch Demo Video**](https://youtu.be/F1DYeII_khY)

#### **CAE Aviation Technician Training**
* **Role:** Senior XR Developer
* **Stack:** Unity, Photon Fusion 2, Hand Tracking, iPad Cross-platform.
* **Description:** A collaborative training suite for aircraft mechanics. Implemented a robust multiplayer system and cross-platform connectivity between VR and mobile devices.
* [**Watch Demo Video**](https://youtu.be/2CuAvT7CWWI)

#### **Embraer Commercial Aircraft Simulator**
* **Role:** Lead Developer (Refactoring & Multiplayer)
* **Stack:** Unity, Photon Fusion 2.
* **Description:** Focused on complex aircraft systems (engines, cockpit, ailerons). I refactored the core interaction systems and implemented a synchronized multiplayer environment for collaborative learning.
* [**Watch Demo Video**](https://youtu.be/Ob_-DYVQqes)

---

### 🏛️ Published on Meta Store

#### **Molecule Builder (Educational Chemistry)**
* **Role:** Lead Developer
* **Status:** Published on Meta Store
* **Description:** An interactive chemistry experience allowing users to assemble molecules while exploring Lewis structures, chemical bonds, and polarity in an intuitive 3D environment.
* [**Watch Demo Video**](https://youtu.be/N8M7DsjTeM8)

#### **Biology Dissection Lab**
* **Role:** Senior Developer (Refactoring & Optimization)
* **Status:** Published on Meta Store
* **Description:** Contributed by refactoring key gameplay mechanics and resolving critical bugs, significantly improving stability and performance for the final release.
* [**Watch Demo Video**](https://youtu.be/2H5K-kex1zw)

---

### 🌊 Research & Social Impact

#### **University of Miami: Rescue a Reef**
* **Role:** Lead Developer
* **Stack:** **Unreal Engine**
* **Description:** Created for the University of Miami to educate users about coral conservation along the Florida coastline. Overseeing both technical implementation and environment development.
* [**Watch Demo Video**](https://youtu.be/ubsAcl7sWe0)

#### **Boys & Girls Clubs: Construction VR**
* **Role:** Lead Developer
* **Description:** A vocational VR experience designed to educate young people in the US about careers in the construction industry.
* [**Watch Demo Video**](https://youtu.be/6kP7dtGdkh4)

#### **VR Optics Table: Snell's Law**
* **Description:** A physics-focused simulation of light refraction and reflection, applying Snell's Law within a virtual laboratory setting.
* [**Watch Demo Video**](https://youtu.be/rNuKt9C5At8)

---

## 🛠 Technical Skills

* **Game Engines:** Unity (Expert), Unreal Engine (Advanced).
* **Languages:** C#, C++, Blueprints.
* **XR Frameworks:** Meta Quest SDK (Oculus), OpenXR, Hand Tracking.
* **Networking/Multiplayer:** Photon Fusion 2, Photon Realtime, Mirror.
* **AI & Interactivity:** OpenAI Realtime API, ElevenLabs AI, Custom Interaction Frameworks.
* **Optimization:** Draw call reduction, Profiling (CPU/GPU), URP/HDRP, Standalone (Quest) performance tuning.
